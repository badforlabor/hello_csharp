...
namespace crazy_collects
{
    ...
    public class CrazyHashSet<T> : ICollection<T>, ISerializable, IDeserializationCallback, ISet<T>, IReadOnlyCollection<T>
    {
        ...
        public CrazyHashSet(int capacity)
            : this(capacity, EqualityComparer<T>.Default) { }

        public CrazyHashSet(IEqualityComparer<T> comparer)
        {
            if (comparer == null)
            {
        ...
        public CrazyHashSet(IEnumerable<T> collection)
        ...
        public CrazyHashSet(IEnumerable<T> collection, IEqualityComparer<T> comparer)
            : this(comparer)
        {
            if (collection == null)
            {
            ...
            var otherAsHashSet = collection as CrazyHashSet<T>;
        ...
        // equality comparer.
        private void CopyFrom(CrazyHashSet<T> source)
        {
        ...
        protected CrazyHashSet(SerializationInfo info, StreamingContext context)
        {
        ...
        public CrazyHashSet(int capacity, IEqualityComparer<T> comparer)
            : this(comparer)
        {
            if (capacity < 0)
            {
        ...
        public virtual void OnDeserialization(Object sender)
        {
            ...
            if (capacity != 0)
            {
                ...
                if (array == null)
                {
                    throw new SerializationException(SR.GetString(SR.ESr.Serialization_MissingKeys));
                }
        ...
        public void IntersectWith(IEnumerable<T> other)
        {
            ...
            if (otherAsCollection != null)
            {
                ...
                CrazyHashSet<T> otherAsSet = other as CrazyHashSet<T>;
        ...
        public void CopyTo(T[] array, int arrayIndex, int count)
        {
            ...
            if (arrayIndex < 0)
            {
                throw new ArgumentOutOfRangeException("arrayIndex", SR.GetString(SR.ESr.ArgumentOutOfRange_NeedNonNegNum));
            }

            // also throw if count less than 0
            if (count < 0)
            {
                throw new ArgumentOutOfRangeException("count", SR.GetString(SR.ESr.ArgumentOutOfRange_NeedNonNegNum));
            }
            ...
            {
                throw new ArgumentException(SR.GetString(SR.ESr.Arg_ArrayPlusOffTooSmall));
            }
        ...
        /// </summary>
        /// <returns></returns>
        public static IEqualityComparer<CrazyHashSet<T>> CreateSetComparer()
        {
        ...
        private void IncreaseCapacity()
        {
            ...
            if (newSize <= m_count)
            {
                throw new ArgumentException(SR.GetString(SR.ESr.Arg_HSCapacityOverflow));
            }
        ...
        /// <param name="other"></param>
        /// <returns></returns>
        private bool IsSubsetOfHashSetWithSameEC(CrazyHashSet<T> other)
        {
        ...
        /// </summary>
        /// <param name="other"></param>
        private void IntersectWithHashSetWithSameEC(CrazyHashSet<T> other)
        {
            for (int i = 0; i < m_lastIndex; i++)
            {
                if (m_slots[i].hashCode >= 0)
        ...
        /// </summary>
        /// <param name="other"></param>
        private void SymmetricExceptWithUniqueHashSet(CrazyHashSet<T> other)
        {
            foreach (T item in other)
            {
                if (!Remove(item))
        ...
        /// <param name="comparer"></param>
        /// <returns></returns>
        internal static bool HashSetEquals(CrazyHashSet<T> set1, CrazyHashSet<T> set2, IEqualityComparer<T> comparer)
        {
        ...
        /// <param name="set2"></param>
        /// <returns></returns>
        private static bool AreEqualityComparersEqual(CrazyHashSet<T> set1, CrazyHashSet<T> set2)
        {
        ...
        {
            private CrazyHashSet<T> set;
            private int index;
            private int version;
            private T current;

            internal Enumerator(CrazyHashSet<T> set)
            {
                this.set = set;
                index = 0;
            ...
            Object System.Collections.IEnumerator.Current
            {
                get
                {
                    ...
                    {
                        throw new InvalidOperationException(SR.GetString(SR.ESr.InvalidOperation_EnumOpCantHappen));
                    }
                    return Current;
                }
            }
    ...
    internal class HashSetEqualityComparer<T> : IEqualityComparer<CrazyHashSet<T>>
    {
        ...
        public bool Equals(CrazyHashSet<T> x, CrazyHashSet<T> y)
        {
            return CrazyHashSet<T>.HashSetEquals(x, y, m_comparer);
        }

        public int GetHashCode(CrazyHashSet<T> obj)
        {
            int hashCode = 0;
            if (obj != null)
            {
